			 ...
			 const struct fsverity_enable_arg *arg)
{
	...
	if (arg->salt_size &&
	    copy_from_user(desc->salt, (const void __user *)u64_to_user_ptr((u64)arg->salt_ptr),
			   arg->salt_size)) {
		err = -EFAULT;
		goto out;
	}
	...
	if (arg->sig_size &&
	    copy_from_user(desc->signature, (const void __user *)u64_to_user_ptr((u64)arg->sig_ptr),
			   arg->sig_size)) {
		err = -EFAULT;
		goto out;
	}
